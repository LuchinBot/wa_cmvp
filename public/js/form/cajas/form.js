...
dialog.create({
    selector: "#form-"+_path_controller_caja
    ...
    ,buttons: {
        Guardar: function(){
            form.get(_path_controller_caja).guardar();
        },
        Cancelar: function(){
            form.get(_path_controller_caja).cancelar();
        }
    }
    ,close: function() {
        form.get(_path_controller_caja).reset();
    }
});


form.register(_path_controller_caja, {
    _prefix: "",
	_form: "#form-"+_path_controller_caja,
    ...
    editar: function(id) {
		var $self = this;
        $.ajax({
			url:route(_path_controller_caja+'.edit',id),
			type:'GET',
            beforeSend: function() {
                loading();
			...
			success: function(response) {
                ...
                $("#codtipo_doc_ident"+_prefix_caja).val(response.persona_natural.codtipo_documento_identidad);
                $("#nro_doc_caja"+_prefix_caja).val(response.persona_natural.numero_documento_identidad);
                $("#nombres_caja"+_prefix_caja).val(response.persona_natural.nombre_completo);
    ...
    eliminar: function(id) {
        ...
        ventana.confirm({
			...
			if(ok) {
                $.ajax({
                    url:route(_path_controller_caja+'.destroy',id),
                    type:'DELETE',
                    beforeSend: function() {
                        loading();
                    },
    ...
    guardar: function(){
		...
		if(s){
            ...
            $.ajax({
                url: route(_path_controller_caja+'.store'),
                type: 'POST',
                data: post_data,
                cache       : false,
                contentType : false,
                ...
                error: function(e){
                    if(e.status==422){ //Errores de Validacion
                        ...
                        toastr.error((e.responseJSON.message)??'Hay una caja abierta, debes cerrarla para poder abrir una nueva.');
                    }
                }
            });
        }
...
limpieza_caja=(id)=>{
    id = id || "#form-"+_path_controller_caja;
