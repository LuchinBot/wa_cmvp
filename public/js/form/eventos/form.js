...
var myDropzone = new Dropzone("#dropzone"+_prefix_evento, {
    ...
    url: route(_path_controller_evento+'.upload_file'),
    headers: {
...
$('#descripcion'+_prefix_evento).summernote({
...
dialog.create({
    selector: "#form-"+_path_controller_evento
    ,title: "Registrar "+_name_module_evento
    ,width: 'modal-md'
    ,style: 'primary'
    ,closeOnEscape: true
    ,buttons: {
        Guardar: function(){
            form.get(_path_controller_evento).guardar();
        },
        Cancelar: function(){
            form.get(_path_controller_evento).cancelar();
        }
    }
    ,close: function() {
        form.get(_path_controller_evento).reset();
    }
});

form.register(_path_controller_evento, {
    _prefix: _prefix_evento,
	_form: "#form-"+_path_controller_evento,

    nuevo: function() {
        limpieza_evento(this._form);
    ...
    editar: function(id) {
		var $self = this;
        $.ajax({
			url:route(_path_controller_evento+'.edit',id),
			type:'GET',
            beforeSend: function() {
                loading();
    ...
    eliminar: function(id) {
        ...
        ventana.confirm({
			...
			if(ok) {
                $.ajax({
                    url:route(_path_controller_evento+'.destroy',id),
                    type:'DELETE',
                    beforeSend: function() {
                        loading();
                    },
                    success: function(response) {
                        toastr.success('Datos grabados correctamente.','Notificación '+_name_module_evento);
    ...
    guardar: function(){
		...
		if(s){
            ...
            $.ajax({
                url: route(_path_controller_evento+'.store'),
                type: 'POST',
                data: post_data,
                beforeSend: function() {
                    loading();
    ...
    callback: function(data) {
		grilla.reload(_name_tabla_evento);
	},
    reset: function() {
        $(":input", this._form).val("");
        $('.nav-tabs a[href="#custom-tabs-01"]').tab('show');
        $("#descripcion", this._form).summernote('code', "<p><br></p>");
        $("#prev_imagen").attr("src", _default_imagen_evento);
...
limpieza_evento=(id)=>{
    id = id || "#form-"+_path_controller_evento;
